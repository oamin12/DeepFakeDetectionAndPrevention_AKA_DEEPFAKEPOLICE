        ...
        with open(checkpoint_file, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    row = json.loads(line)
                    done_ids.add(row["id"])
        tqdm.write(f"{progress_prefix}Resuming from checkpoint: {len(done_ids)} images already processed")

    remaining = [x for x in samples if x["id"] not in done_ids]
    tqdm.write(f"{progress_prefix}Remaining images to process: {len(remaining)}")
